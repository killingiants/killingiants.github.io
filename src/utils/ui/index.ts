...
import ripple, {rippleCut, rippleReverse} from './ripple';
...
const isColor = (color: string) => {
  ...
  return tdColors.includes(color);
};

const setVar = (propertyName: string, value: string, el: any) => {
  if (!el) {
    document.documentElement.style.setProperty(`--td-${propertyName}`, value);
  } else {
    if (el.nodeName !== '#comment') {
      el.style.setProperty(`--td-${propertyName}`, value);
    }
  }
};
...
const setColor = (colorName: string, color: string, el: any, addClass?: boolean) => {
  ...
  if (color == 'dark' && el) {
    if (addClass) {
      el.classList.add('td-component-dark');
    }
  }
  ...
  if (isRGB) {
    ...
    if (addClass) {
      el.classList.add('td-change-color');
    }
  } else if (isHEX) {
    ...
    newColor = style.getPropertyValue('--td-' + color);
    setVar(colorName, newColor, el);
    if (addClass) {
      el.classList.add('td-change-color');
    }
    ...
    console.warn({
      ...
      text: `• Component: ${el.__vue__.$vnode.componentOptions.tag}
        • Warn info: The color format is incorrect
        • Prop: color
        • value: ${color}
        • Permited values: (RGB, HEX, RGB Numbers, td Colors)
