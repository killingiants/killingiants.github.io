...
const ripple = (evt: any, color: any = '', solid = false) => {
  ...
  effectContent.className = 'td-ripple-content';

  const effect = document.createElement('div');

  if (!color) setColor('color', '#fff', effectContent);

  effect.className = 'td-ripple';
  if (solid) {
    effect.classList.add('td-ripple--solid');
...
const rippleReverse = (evt: any) => {
  ...
  effect.className = 'td-ripple-invert';
  // effect.style.transition = `all ${time}s ease`
...
const rippleCut = (evt: any) => {
  ...
  effect.className = 'td-ripple-cut-1';
  effect2.className = 'td-ripple-cut-2';
  // effect.style.transition = `all ${time}s ease`
